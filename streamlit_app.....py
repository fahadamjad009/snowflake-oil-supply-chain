...
df_balance = session.table("OIL_SUPPLY_CHAIN.MARTS.FCT_SUPPLY_BALANCE").to_pandas().sort_values("PERIOD_DATE")
df_balance["PERIOD_DATE"] = pd.to_datetime(df_balance["PERIOD_DATE"])

df_prod = session.table("OIL_SUPPLY_CHAIN.MARTS.FCT_GLOBAL_PRODUCTION").to_pandas()

df_cushing_all = session.table("OIL_SUPPLY_CHAIN.MARTS.FCT_STORAGE_CUSHING").to_pandas()
df_cushing_all["PERIOD_DATE"] = pd.to_datetime(df_cushing_all["PERIOD_DATE"])
df_cushing = df_cushing_all[df_cushing_all["DUOAREA"] == "YCUOK"].sort_values("PERIOD_DATE")

df_grade = session.table("OIL_SUPPLY_CHAIN.MARTS.FCT_CRUDE_IMPORTS_BY_GRADE").to_pandas()
df_grade["PERIOD_DATE"] = pd.to_datetime(df_grade["PERIOD_DATE"])

df_moves = session.table("OIL_SUPPLY_CHAIN.MARTS.FCT_INTERREGIONAL_MOVEMENTS").to_pandas()
df_moves["PERIOD_DATE"] = pd.to_datetime(df_moves["PERIOD_DATE"])
...
# ---------- Matplotlib-free heatmap styling ----------
def style_heatmap(df, low_color=(255, 255, 204), high_color=(189, 0, 38)):
    """Cell-level background-color CSS, computed manually so no matplotlib
    dependency is required (avoids the same PyPI-install flakiness plotly hit)."""
